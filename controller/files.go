...
// @Success 200 {object} _ResponseMsg "成功上传问题文件"
// @Failure 403 {object} _ResponseMsg "Token 已超时或用户非管理员"
// @Failure 400 {object} _ResponseMsg "接收或保存文件出错"
// @Router /problem/file/add/:id [post]
func AddFilesToProblem(c *gin.Context) {
	var err error
	// 从请求中获取执行操作的管理员用户名
	var file File
	// 从请求中获取执行操作的用户名
...
// GetProblemFiles 获取问题文件列表
// @Summary 获取问题文件列表
// @Description 通过问题ID，获取指定问题的文件列表
// @Accept json
// @Produce json
// @Param id formData string true "要获取文件列表的问题ID"
// @Success 200 {object} _ResponseGetData "成功获取问题文件列表"
// @Failure 403 {object} _ResponseDataString "读取文件列表出错"
// @Router /problem/file/:id [post]
func GetProblemFiles(c *gin.Context) {
	// 从请求中获取要获取文件列表的问题ID
